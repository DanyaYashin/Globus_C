...
export default function Home() {
  ...
  const reviews = [
    "Хочу выразить огромную благодарность нашему бухгалтеру! Более 5 лет сотрудничаю и ни разу не было проблем с отчетностью. Всегда всё сдается вовремя, налоги считает оптимально, консультирует по любым вопросам. Настоящий профессионал своего дела!",
    "Работаем уже третий год и очень довольны. Бухгалтер ведет несколько наших компаний, всегда всё четко и организовано. Никаких штрафов и просрочек. Особенно ценно, что всегда на связи и оперативно решает любые вопросы.",
    "Как же я рада, что нашла такого специалиста! Раньше постоянно были проблемы с налоговой, теперь всё идеально. Объясняет сложные вещи простым языком, всегда подскажет как лучше поступить. Настоящее спасение для моего бизнеса!",
    "Бухгалтер ведет наш учет уже больше 4 лет. Ни единого замечания от ИФНС за все это время! Все отчеты сдаются день в день, налоги оптимизированы. Очень ответственный и внимательный к деталям человек.",
    "Это лучший бухгалтер с которым я работала! Всегда вежлива, профессиональна, находит решения для сложных ситуаций. Благодаря ей я могу полностью сосредоточиться на развитии бизнеса, не думая о бумажной работе. Очень рекомендую!"
  ];
          ...
          <div className="my-10 md:mt-0 w-4/5 md:w-3/4 xl:w-2/3 h-60vh bg-gradient-to-b from-[#14281D] from-30% to-[#E0C28D] mx-auto border-hidden rounded-xl shadow-lg relative">
            <div className="w-full h-full px-6 pt-6 ">
              ...
              <div className="news-list space-y-4 overflow-y-scroll h-48vh scrollbar-hide ">
                ...
                {[
                  "С 1 января 2024 года изменились лимиты по УСН и патентам. Теперь предприниматели могут применять упрощенку при доходе до 250 млн рублей и численности сотрудников до 150 человек.",
                  "Налоговая служба упростила процесс сдачи электронной отчетности для малого бизнеса. Теперь доступен новый личный кабинет с улучшенным интерфейсом и автоматической проверкой документов.",
                  "Введены новые коды КВЭД для IT-компаний и специалистов удаленного формата работы. Рекомендуем проверить актуальность ваших кодов деятельности.",
                  "С 1 марта начинается кампания по сдаче годовой отчетности за 2023 год. Напоминаем о сроках: ООО — до 31 марта, ИП — до 30 апреля.",
                  "Изменен порядок выплат дивидендов учредителям. Теперь требуется предоставлять дополнительные документы для подтверждения законности операций.",
                  "Запущен пилотный проект по автоматическому возмещению НДС для компаний из сферы экспорта. Ускоренный процесс занимает всего 10 рабочих дней.",
                  "Введены новые меры поддержки малого бизнеса: налоговые каникулы для впервые зарегистрированных ИП в сфере образования и здравоохранения."
                ].map((news, index) => (
                  <div
                    key={index}
